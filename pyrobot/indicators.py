...
class Indicators():
    ...
    def average_true_range(self, period: int = 14, column_name: str = 'average_true_range') -> pd.DataFrame:
        ...
        self._frame['true_range'] = self._frame[[
            'true_range_0', 'true_range_1', 'true_range_2']].max(axis=1)
        ...
        # Clean up before sending back.
        self._frame.drop(
            labels=['true_range_0', 'true_range_1',
                    'true_range_2', 'true_range'],
            axis=1,
            inplace=True
        )
    ...
    def macd(self, fast_period: int = 12, slow_period: int = 26, column_name: str = 'macd') -> pd.DataFrame:
        ...
        self._frame['macd_diff'] = self._frame['macd_fast'] - \
    ...
    def mass_index(self, period: int = 9, column_name: str = 'mass_index') -> pd.DataFrame:
        ...
        # Grab the raw index.
        self._frame['mass_index_raw'] = self._frame['mass_index_1'] / \
            self._frame['mass_index_2']
    ...
    def force_index(self, period: int, column_name: str = 'force_index') -> pd.DataFrame:
        ...
        self._frame[column_name] = self._frame['close'].diff(
            period) * self._frame['volume'].diff(period)
    ...
    def ease_of_movement(self, period: int, column_name: str = 'ease_of_movement') -> pd.DataFrame:
        ...
        high_plus_low = (self._frame['high'].diff(
            1) + self._frame['low'].diff(1))
        diff_divi_vol = (
            self._frame['high'] - self._frame['low']) / (2 * self._frame['volume'])
    ...
    def commodity_channel_index(self, period: int, column_name: str = 'commodity_channel_index') -> pd.DataFrame:
        ...
        self._frame[column_name] = self._frame['typical_price_mean'] / \
            self._frame['typical_price_std']
